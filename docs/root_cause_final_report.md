...
**الحالة الموثقة:** commit `2dab6b1` على `main`، وخدمة Render الحالية `https://mkmoon.onrender.com`.
...
آخر تحقق إنتاجي مستقر أعاد أن الخدمة online وPostgreSQL وRedis متصلان، وأن آخر دورة مكتملة عالجت 25/25 رمزًا، وأنشأت 25 قرارًا، وسجلت 0 أخطاء و0 أوامر، مع `WATCH=25` و`ENTER=0`. سبب المنع المباشر في هذه الدورة هو `SETUP_INCOMPLETE=25`؛ وتبقى بوابة EV مغلقة عمدًا كحاجز لاحق حتى تتوافر معايرة موثقة.[3] [4]
...
| `disabled_until_calibrated` كان يمنع الدخول بصمت | لا يعرف المشغل لماذا لا تنشأ الصفقات | أُبقي المنع عمدًا، مع حالة وسبب واضحين؛ لا يُفتح إلا بعد عينة معايرة موثقة | آخر دورة `WATCH=25` و`ENTER=0`؛ السبب المباشر الحالي `SETUP_INCOMPLETE` |
...
| دورة Worker كانت بطيئة ومتسلسلة | توسعة النطاق إلى 25 رمزًا قد ترفع زمن الدورة أو تؤدي إلى إلغاء جزئي | Semaphore محدود على مستوى الدورة، مهلات HTTP، وتصنيف الإلغاء كـFAILED | آخر دورة مكتملة 25/25 دون أخطاء؛ الزمن يتغير حسب المصدر والـfallback |
...
لا يُفحص خروج المركز من شمعة سبقت فتحه. في كل دورة لاحقة، تُفحص جميع شموع 5m المغلقة بعد `opened_at_ms` بترتيب زمني، ويُسجّل أول وقف أو هدف يحدث. إذا أصابت الشمعة stop وtarget معًا، يطبق النظام قاعدة محافظة: stop أولًا بسبب عدم معرفة ترتيب الحركة داخل الشمعة. عند الخروج، تُنشأ SELL fill، يُغلق الأمر والمركز، يُحرر reservation، ويُحسب صافي PnL بعد رسوم الدخول والخروج، مع lifecycle events `EXIT_REQUEST` و`CLOSED`.
...
هذا السلوك مقصود وآمن، وليس عطلًا في Worker. في آخر دورة لم يكتمل أي setup أصلًا، فكان `SETUP_INCOMPLETE` هو الحاجز المباشر. بعد اكتمال setup ستظل سياسة `edge.disabled_until_calibrated` تمنع الدخول قبل امتلاك عينة نتائج Paper كافية ومعايرة قابلة للإثبات. لذلك لا يجوز تغييرها إلى enabled لمجرد رؤية أوامر في Dashboard. المسار العلمي التالي، خارج هذا الإصلاح، هو جمع نتائج Paper، التحقق من أن كل fill مرتبط بقرار ومصدر ونسخة config، ثم حساب العينة وEV على بيانات نتائج حقيقية. أي فتح للبوابة يجب أن يكون تغيير سياسة مستقلًا ومراجعًا، وليس تعديلًا صامتًا أثناء التشغيل.

## مصادر البيانات والـ25 أصلًا

واجهة Dashboard تعرض قائمة 25 زوجًا وتستقبل أسعار العرض من Binance WebSocket العام في المتصفح، مع REST fallback. هذه الأسعار للعرض وليست مصدرًا لأوامر التنفيذ. Worker الإنتاجي يعمل حاليًا على 25 رمزًا؛ تم التحقق من القائمة على المصادر العامة وتحديث Environment اليدوي في Render. قد يظهر `https://api.binance.us` كمصدر فعلي مستقل عند فشل المضيفين الأساسيين، وليس نسخة مطابقة من Binance.com.
...
نفذت النواة `compileall` بنجاح، ونجحت مجموعة الاختبارات الحالية بنتيجة **32 passed, 1 skipped** بعد إضافة اختبارات JSONB وPaper exit وتصنيف الدورات وواجهة RTL. تشمل الاختبارات الحالية fallback العام، حماية private client من التداول في Paper، محاكاة entry/exit المحافظة، وتقريب المخاطر وهندسة الأسعار. كما طُبقتا migration 0006 و0007 على Supabase، وتأكدت عقود الجداول بقراءة `information_schema` دون تعديل بيانات تشغيلية.

تسجل هذه الفقرة تحققًا تاريخيًا من deploy `6401d65` ومشكلة `AmbiguousParameterError` التي عولجت لاحقًا في commit `008e52c` بإضافة casts إلى `bigint`. أما الحالة الحالية بعد الإصدار `2dab6b1` فموثقة في سجل التحقق اللاحق: `/health` production/Paper، و`/ready` ready، وآخر دورة مكتملة 25/25 دون أخطاء.[1] [2] [3]

## ما بقي مقصودًا أو يحتاج قرارًا مستقلًا

| البند المتبقي | السبب | الإجراء الآمن التالي |
|---|---|---|
| لا توجد Paper orders إنتاجية حتى الآن | آخر دورة لم تكتمل فيها أي استراتيجية، فكان `SETUP_INCOMPLETE` هو الحاجز المباشر؛ وبعد اكتمال setup ستبقى بوابة EV حاجزًا لاحقًا | لا تفتح البوابة ولا تخفف الشروط لمجرد إنشاء صفقة؛ اجمع عينة معايرة Paper أولًا |
| دورة قد تُقطع أثناء deploy أو restart | قد تُحفظ أرقام جزئية مثل 10/25 أو 20/25 كسجل مضلل إذا لم تُصنّف الإلغاء | الإصدار `2dab6b1` يسجل الدورة الملغاة FAILED بدل COMPLETED، ويجب اعتبار آخر دورة COMPLETED فقط عند التحقق |
...
| آخر commit والتحقق | [commit 2dab6b1][7] |
...
النظام الآن صالح للتشغيل **Paper-only** والتحليل الدوري القابل للتدقيق، وليس صالحًا ولا مهيأً للتداول الحي، وهذا مقصود. تم إصلاح عيوب التنفيذ الورقي، provenance، التزامن، الاسترداد، المخاطر، الأداء، وCycle Summary من جذورها ضمن الكود والمخطط. أما فتح الدخول أو الانتقال إلى Binance private/live فهي تغييرات تشغيلية جديدة تتطلب تحققًا منفصلًا وموافقة صريحة؛ نطاق 25 رمزًا أصبح مفعلًا ومتحققًا في Render، ولا ينبغي تفسير غياب الأوامر الحالية كفشل في معالجة الرموز.
...
[7]: https://github.com/mkkh01/Mkmoon/commit/2dab6b1 "Mark interrupted paper cycles as failed"
